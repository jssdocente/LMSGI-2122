...
* La **cascada** la cascada en las hojas de estilo significa que el orden de las reglas importa en CSS: cuando dos reglas tienen la misma especificidad, se aplica la que aparece en último lugar en el CSS, y además si se aplican propiedades iguales sobre un elemento prevalece la última que se aplica, y será la que esté más abajo en la cascada, es decir, más abajo en el fichero CSS.
